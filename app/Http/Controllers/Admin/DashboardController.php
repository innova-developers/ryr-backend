...
class DashboardController extends Controller
{
    public function stats(Request $request): JsonResponse
    {
        ...
        if ($dateFrom) $commissionsQuery->where('date', '>=', $dateFrom);
        if ($dateTo) $commissionsQuery->where('date', '<=', $dateTo);

        $totalCommissions = (clone $commissionsQuery)->count();

        $customerIdsQuery = Commission::query()->select('client_id');
        if ($dateFrom) $customerIdsQuery->where('date', '>=', $dateFrom);
        if ($dateTo) $customerIdsQuery->where('date', '<=', $dateTo);
        ...
        if ($dateFrom) $expensesQuery->where('date', '>=', $dateFrom);
        if ($dateTo) $expensesQuery->where('date', '<=', $dateTo);
